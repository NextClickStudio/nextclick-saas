...
(function() {
  ...
  function init() {
    // 1. Cerca prima un <div data-brand="..."> esplicito (NON script)
    var container = document.querySelector('div[data-brand]');

    // 2. Se non c'è, cerca lo script con data-brand e crea un div prima di lui
    if (!container) {
      var scriptTag = document.querySelector('script[data-brand]');
      if (!scriptTag) return;
      container = document.createElement('div');
      container.setAttribute('data-brand', scriptTag.getAttribute('data-brand'));
      // Inserisce il div PRIMA dello script tag, nel punto esatto della pagina
      scriptTag.parentNode.insertBefore(container, scriptTag);
    }

    var brandId = container.getAttribute('data-brand');
    if (!brandId || brandId === 'IL_TUO_BRAND_ID') {
      console.warn('NextClick: brand ID non valido');
      return;
    }

    fetch(BASE_URL + '/api/config/' + brandId)
      .then(function(r) {
        if (!r.ok) throw new Error('Config non trovata (HTTP ' + r.status + ')');
        return r.json();
      })
      .then(function(config) {
        renderWidget(container, brandId, config, BASE_URL);
      })
      .catch(function(e) {
        console.error('NextClick widget error:', e);
      });
  }

  // Avvia quando il DOM è pronto, compatibile con defer e inline
...
function renderWidget(container, brandId, config, baseUrl) {
  ...
  var pfx = 'nc-' + brandId.replace(/-/g,'').substring(0, 8);
  var fnKey = brandId.replace(/-/g,'_');

  // Font
  if (d.font_family && d.font_family !== 'System') {
    var fontLink = document.createElement('link');
    fontLink.rel = 'stylesheet';
    fontLink.href = 'https://fonts.googleapis.com/css2?family=' + encodeURIComponent(d.font_family) + ':wght@300;400;500;700;800&display=swap';
  ...
  style.textContent = [
    '.nc-widget{background:' + d.color_bg + ';color:' + d.color_text + ';font-family:\'' + font + '\',sans-serif;max-width:680px;margin:0 auto;padding:32px 24px;border-radius:8px;box-sizing:border-box}',
    ...
    '.nc-input{width:100%;padding:12px;border:1px solid rgba(0,0,0,.15);background:rgba(255,255,255,.05);color:' + d.color_text + ';font-family:\'' + font + '\',sans-serif;font-size:.9rem;border-radius:4px;outline:none;margin-bottom:12px;box-sizing:border-box}',
    ...
    '.nc-btn-outline{background:transparent!important;border:1px solid ' + d.color_button + '!important;color:' + d.color_button + '!important;margin-right:8px}',
    ...
    '.nc-product{border:1px solid rgba(0,0,0,.1);padding:16px;border-radius:4px;display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap}',
  ...
  container.innerHTML =
    '<div class="nc-widget" id="' + pfx + '">' +
      '<div id="' + pfx + '-intro" class="nc-step active">' +
        '<div class="nc-title">' + (copy.title || 'Trova i prodotti perfetti per te') + '</div>' +
        '<div class="nc-sub">' + (copy.subtitle || 'Rispondi a poche domande') + '</div>' +
        '<button class="nc-btn" onclick="ncStart_' + fnKey + '()">' + (copy.cta || 'Inizia ora \u2192') + '</button>' +
      '</div>' +
  ...
  window['ncStart_' + fnKey] = function() {
    state.step = 0;
    showQuestion();
  };
  ...
  function showQuestion() {
    ...
    if (q.type === 'choice') {
      inputHtml = q.opts.map(function(o) {
        ...
        return '<button class="nc-btn nc-btn-outline" style="margin:4px" onclick="ncAns_' + fnKey + '(\'' + q.key + '\',\'' + safe + '\')">' + o + '</button>';
      }).join('');
    } else {
      inputHtml =
        '<textarea class="nc-input nc-textarea" id="' + pfx + '-desc" placeholder="Scrivi qui..."></textarea>' +
        '<button class="nc-btn" onclick="ncText_' + fnKey + '(\'' + q.key + '\')">' + (isLast ? 'Genera \u2192' : 'Avanti \u2192') + '</button>';
    }
    ...
    stepEl.innerHTML =
      ...
      '<div>' + inputHtml + '</div>' +
      (state.step > 0 ? '<div style="margin-top:16px"><button class="nc-btn nc-btn-outline" onclick="ncBack_' + fnKey + '">\u2190 Indietro</button></div>' : '');
  }

  window['ncAns_' + fnKey] = function(key, val) {
  ...
  window['ncText_' + fnKey] = function(key) {
    var ta = document.getElementById(pfx + '-desc');
    var val = ta ? ta.value : '';
  ...
  window['ncBack_' + fnKey] = function() {
    state.step = Math.max(0, state.step - 1);
    showQuestion();
  };

  function ncGenerate() {
    var qEl = el('q'); if (qEl) qEl.classList.remove('active');
    var loadEl = el('loading'); if (loadEl) loadEl.classList.add('active');
    ...
    .then(function(data) {
      var loadEl2 = el('loading'); if (loadEl2) loadEl2.classList.remove('active');
      ...
      if (data.error) {
        ...
        resultEl.innerHTML = '<div class="' + cls + '">' + msg + '</div>' +
          '<button class="nc-btn" style="margin-top:16px" onclick="ncRestart_' + fnKey + '()">\u21ba Riprova</button>';
        return;
      }
      ...
      html += '<div style="margin-top:24px"><button class="nc-btn nc-btn-outline" onclick="ncRestart_' + fnKey + '()">\u21ba Ricomincia</button></div>';
      resultEl.innerHTML = html;
    })
    .catch(function(e) {
      var loadEl3 = el('loading'); if (loadEl3) loadEl3.classList.remove('active');
      var resultEl = el('result');
      if (!resultEl) return;
      resultEl.classList.add('active');
      resultEl.innerHTML = '<div class="nc-error">Errore di connessione. Riprova.</div>' +
        '<button class="nc-btn" style="margin-top:16px" onclick="ncRestart_' + fnKey + '()">\u21ba Riprova</button>';
    });
  }

  window['ncRestart_' + fnKey] = function() {
    state.step = 0; state.profile = {}; state.description = '';
    var r = el('result'); if (r) r.classList.remove('active');
    var intro = el('intro'); if (intro) intro.classList.add('active');
  };
}
...
function getQuestions(category) {
  ...
  return cats[category] || cats.beauty;
}
