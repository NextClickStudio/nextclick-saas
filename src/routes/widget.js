...
function getQuestions(category, lang) {
  ...
  var questions = it[category] || it.beauty;
  return questions;
}
...
function showQuestion(brandId, questions, state, baseUrl, plan, config) {
  ...
  window['ncSubmitText'] = function(bid, key) {
    if (bid !== brandId) return;
    var val = document.getElementById('nc-desc')?.value || '';
    if (key === 'description') state.description = val;
...
function ncGenerate(brandId, state, baseUrl, plan, config) {
  document.getElementById('nc-step-q')?.classList.remove('active');
  document.getElementById('nc-step-loading')?.classList.add('active');
  ...
  .then(function(r) { return r.json(); })
  .then(function(data) {
    document.getElementById('nc-step-loading')?.classList.remove('active');
  ...
  window['ncRestart'] = function(bid) {
    ...
    document.getElementById('nc-step-result')?.classList.remove('active');
    document.getElementById('nc-step-intro')?.classList.add('active');
  };
}
