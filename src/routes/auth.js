...
// ── HOME ──
router.get('/', (req, res) => {
  if (req.session && req.session.brandId) return res.redirect('/dashboard');
  res.redirect('/login');
});
...
router.post('/login', (req, res) => {
  ...
  if (!email || !password) {
    return res.redirect('/login?error=Compila+tutti+i+campi');
  }

  const brand = db.get().prepare('SELECT * FROM brands WHERE email = ?').get(email.trim().toLowerCase());
  ...
  // FIX: Set session values then explicitly save before redirect
  // Without session.save(), on Railway the redirect fires before
  // the session is persisted, so requireAuth finds no brandId.
  ...
  req.session.save((err) => {
    if (err) {
      ...
      return res.redirect('/login?error=Errore+interno,+riprova');
    }
...
router.post('/register', (req, res) => {
  ...
  const existing = db.get().prepare('SELECT id FROM brands WHERE email = ?').get(emailNorm);
  ...
  db.get().prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, 'trial', ?)
  `).run(id, emailNorm, hash, name, plan || 'recommend', genLimit);

  req.session.brandId = id;
  req.session.brandName = name;

  // FIX: same session.save() pattern on register
  req.session.save((err) => {
    if (err) {
      console.error('Session save error:', err);
      return res.redirect('/login?error=Account+creato,+effettua+il+login');
    }
...
<div class="card">
  ...
  <form method="POST" action="/login">
    <label>Email</label>
    <input type="email" name="email" placeholder="brand@example.com" required autocomplete="email"/>
