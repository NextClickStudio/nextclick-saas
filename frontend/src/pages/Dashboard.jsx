...
export function Dashboard() {
  ...
  async function handleResetIpLimits() {
    setResettingIp(true);
    setResetMsg('');
    try {
      const { data: d } = await api.post('/shop/reset-ip-limits');
      setResetMsg('✅ IP limits reset — all users can generate again today.');
    } catch (e) {
  ...
  const kpis = [
    ...
    { label: 'Cart clicks',       value: data?.totalPurchases?.toLocaleString() || '0',    icon: '🛒', color: '#007A5E' },
      ...
      <BlockStack gap="400">
        ...
        <Card>
          <BlockStack gap="300">
            <InlineStack align="space-between" blockAlign="center">
              <BlockStack gap="100">
                <Text variant="headingMd">IP Rate Limiting</Text>
                <Text variant="bodySm" tone="subdued">Max 10 generations per IP address per day. Resets at midnight UTC.</Text>
              </BlockStack>
              <Button
            ...
            {resetMsg && (
              <div style={{ padding: '8px 12px', background: resetMsg.startsWith('✅') ? '#F1F8F5' : '#FFF4F4', borderRadius: 6, fontSize: 13 }}>
                {resetMsg}
              </div>
            )}
          </BlockStack>
