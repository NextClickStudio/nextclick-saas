...
export function OnboardingWizard({ shop }) {
  ...
  async function handleSubscribe() {
    ...
    try {
      ...
      // If we get a confirmationUrl that is the app itself, just reload
      if (data.confirmationUrl && data.confirmationUrl.startsWith('/')) {
    ...
    <div style={{ minHeight: '100vh', background: '#F6F6F7' }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ marginTop: 32, display: 'flex', justifyContent: 'space-between' }}>
          ...
          {step < STEPS.length - 1 ? (
            <Button variant="primary" onClick={next} loading={loading} disabled={step === 3 && !synced}>
