...
async function saveOAuthState(state, shop) {
  await prisma.shopSession.upsert({
    ...
    create: { id: 'oauth_state_' + state, shopDomain: shop, payload: { state, shop, createdAt: Date.now() }, updatedAt: new Date() },
  });
}
...
router.get('/', async (req, res) => {
  try {
    ...
    logger.info('OAuth start', { shop });
    res.redirect(authUrl);
  } catch (err) {
    logger.error('Auth begin failed', err);
...
router.get('/callback', async (req, res) => {
  ...
  try {
    ...
    if (!savedShop || savedShop !== shop) {
      logger.error('OAuth state mismatch', null, { savedShop, shop });
    ...
    logger.info('OAuth complete', { shop, tokenPrefix: accessToken.substring(0, 8) });
    ...
    logger.error('Auth callback failed', err, { shop });
