...
router.post('/sync', verifyShopifySession, async (req, res) => {
  try {
    ...
    // Verifica che il token abbia i permessi leggendo l'endpoint shop prima
...
router.post('/import-url', verifyShopifySession, async (req, res) => {
  try {
    ...
    // Estrai l'handle dal URL (es. https://mio-store.myshopify.com/products/nome-prodotto)
    ...
    if (!session || !session.accessToken) {
      return res.status(401).json({ error: 'Sessione non valida. Reinstalla l\'app.' });
    }

    const client = new shopify.clients.Rest({ session });
    const response = await client.get({
      path: 'products',
      query: { handle, fields: 'id,title,handle,variants,images,tags,product_type,body_html', limit: 1 },
    });
