...
router.post('/ai-tag', verifyShopifySession, async (req, res) => {
  try {
    ...
    if (products.length === 0) {
      return res.status(400).json({ error: 'Nessun prodotto da taggare. Sincronizza prima i prodotti.' });
    }
    ...
    for (const product of products) {
      try {
        const prompt = `Analizza questo prodotto e restituisci SOLO un oggetto JSON valido, nessun altro testo.

Prodotto: ${product.title}
Descrizione: ${product.description || 'non disponibile'}
Tag esistenti: ${(product.tags || []).join(', ') || 'nessuno'}
Categoria: ${product.category || 'non specificata'}

Restituisci JSON con questi campi:
{
  "goals": ["array di obiettivi del prodotto, scegli tra: Idratazione, Anti-età, Luminosità, Imperfezioni, Lenitivo, Purificante, Rassodante, Protezione solare, Nutrizione, Riparazione, Uniformante, Anti-rossore"],
  "skinTypes": ["array di tipi di pelle adatti, scegli tra: Secca, Normale, Grassa, Mista, Sensibile, Atopica, Matura, Tutti i tipi"],
  "allergyFree": ["array di caratteristiche, scegli tra: Senza profumo, Senza parabeni, Senza alcol, Senza siliconi, Vegan, Cruelty-free, Senza nichel, Senza glutine, Ipoallergenico"],
  "inci": ["max 3 ingredienti chiave estratti dalla descrizione, es: Acido ialuronico, Vitamina C, Retinolo"]
}
...
Rispondi SOLO con il JSON.`;
        ...
        // Unisci allergyFree e inci nei tags esistenti
        const newTags = [
          ...(product.tags || []),
          ...(data.allergyFree || []),
          ...(data.inci || []),
      ...
      // Piccola pausa per non saturare le API
      await new Promise(r => setTimeout(r, 200));
    }

    res.json({ tagged, total: products.length, errors: errors.length > 0 ? errors : undefined });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
