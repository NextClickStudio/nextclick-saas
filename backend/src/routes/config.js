...
router.patch('/', verifyShopifySession, async (req, res) => {
  try {
    const allowed = [
      ...
      'textColor', 'buttonColor', 'buttonTextColor', 'fontFamily', 'borderRadius',
  ...
  try {
    const config = await prisma.shopConfig.update({
      ...
      data: {
        ...
        fontFamily: 'Inter',
        borderRadius: '4px',
        heroTitle: 'Trova la tua routine perfetta',
        heroSubtitle: 'Personalizzata per te dall\'intelligenza artificiale',
        ctaText: 'Inizia ora',
        questionFlow: [],
        aiPersona: 'Sei un esperto consulente. Fornisci consigli personalizzati e professionali.',
      },
    });
    res.json({ config });
  } catch (err) {
