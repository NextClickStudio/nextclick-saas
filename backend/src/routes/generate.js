...
router.post('/', verifyWidgetRequest, async (req, res) => {
  ...
  try {
    ...
    const products = await prisma.product.findMany({
      ...
      select: { shopifyId: true, variantId: true, title: true, url: true, imageUrl: true, price: true, category: true, goals: true, tags: true },
    });
    ...
    const variantMap = {};
    products.forEach(p => { if (p.variantId) variantMap[p.shopifyId] = p.variantId; });
