...
const { checkLimit, increment, getClientIP, MAX_PER_IP } = require('../utils/rateLimiter');
...
router.post('/', verifyWidgetRequest, async (req, res) => {
  ...
  try {
    ...
    if (!isPreview) {
      ...
      const { allowed, used, remaining } = await checkLimit(ip, shop.id);
      if (!allowed) {
        return res.status(429).json({
          error: 'ip_rate_limit',
          message: `Daily limit of ${MAX_PER_IP} generations reached for your IP. Resets at midnight UTC.`,
          used,
          limit: MAX_PER_IP,
          remaining: 0,
        });
    ...
    const products = await prisma.product.findMany({
      ...
      select: { shopifyId: true, variantId: true, handle: true, title: true, url: true, imageUrl: true, price: true, category: true, goals: true, tags: true },
    });
    ...
    // Costruisci mappa variantId prima del prompt
    const variantMap = {};
    const handleMap = {};
    products.forEach(p => {
      if (p.variantId) variantMap[p.shopifyId] = p.variantId;
      if (p.handle) handleMap[p.shopifyId] = p.handle;
    });
    ...
    const model = genAI.getGenerativeModel({
      ...
      generationConfig: {
        maxOutputTokens: isRoutine ? 8192 : 4096,
        temperature: 0.3,
      },
    }, { timeout: 120000 });
    ...
    try {
      ...
      logger.error('Generation parse error', e, { shop: shop.shopDomain, rawPreview: rawText?.substring(0, 200) });
    ...
    // Arricchisci i prodotti con variantId dal DB
    const enrichedProducts = (parsed.products || []).map(p => ({
      ...p,
      variantId: variantMap[p.shopifyId] || null,
      handle:    handleMap[p.shopifyId]  || null,
    }));

    // Incrementa contatore IP solo se generazione riuscita
    if (req._ip) await increment(req._ip, shop.id);

    res.json({ generationId: generation.id, type: isRoutine ? 'routine' : 'products', routine: parsed.routine || null, sections: parsed.sections || null, products: enrichedProducts, shopDomain: shop.shopDomain, message: parsed.message || null });

  } catch (err) {
    logger.error('Generation failed', err, { shop: shop.shopDomain });
...
router.post('/purchase', async (req, res) => {
  try {
    const { generationId, productsCount } = req.body;
    if (!generationId || generationId.startsWith('preview-')) return res.json({ ok: true });
    await prisma.generation.update({
      where: { id: generationId },
      data: { purchaseCount: { increment: productsCount || 1 } },
    });
    res.json({ ok: true });
...
function buildPrompt(config, answers, products, isRoutine) {
  ...
  // Map shopifyId → variantId for cart
